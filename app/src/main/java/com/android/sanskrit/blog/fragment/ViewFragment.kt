...
import kotlinx.android.synthetic.main.blog_view_fragment.viewRecycleView
...
/**
 * created by jiangshide on 2020/5/9.
 * email:18311271399@163.com
 */
class ViewFragment(private val blogId: Long = 0) : MyFragment() {
  ...
  override fun onCreateView(
    ...
    return setView(R.layout.blog_view_fragment, true, true)
  }
  ...
  override fun onViewCreated(
    ...
    userVM!!.view.observe(this, Observer {
      hiddle()
      cancelRefresh()
      if (it.error != null) {
        if (adapter == null || adapter?.count() == 0 || blogVM!!.isRefresh) {
          noNet("暂无预览!").setTipsRes(R.mipmap.no_data)
        }
        return@Observer
      }
      showView(it.data)
  ...
  private fun showView(data: MutableList<User>) {
    ...
    adapter = viewRecycleView.create(data, R.layout.blog_view_fragment_item, {
      val user = it
