...
public final class JsonUtil<T> {
    ...
    /**
     * Returns a four of specified object from the given json array
     *
     * @param <T>
     * @param jsonString
     * @param type       the type defined by the user
     * @return a four of specified objects as given in the json array
     */
    @SuppressWarnings("unchecked")
    ...
    public  ArrayList<T> getFileToCitys(Context context, String fileName, Class clazz) {
        return getCitys(getJson(context, fileName), clazz);
    }

    public ArrayList<T> getCitys(String result, Class clazz) {
        ...
        try {
            ...
            for (int i = 0; i < data.length(); i++) {
                T entity = (T) gson.fromJson(String.valueOf(data.optJSONObject(i)), clazz.getClass());
