...
public class VideoProcessor {
  ...
  public static String cutVideo(String sourcePath,int startTimeMs,
                              int endTimeMs) throws Exception {
    Context context = AppUtil.getApplicationContext();
    String outPath = FileUtil.getVideoPath(FileUtil.getFileName(sourcePath));
    cutVideo(context, Uri.parse(sourcePath), outPath, startTimeMs, endTimeMs);
    return outPath;
  }

  public static void cutVideo(String sourcePath, String outPath, int startTimeMs,
      int endTimeMs) throws Exception {
    Context context = AppUtil.getApplicationContext();
  ...
  public static void cutVideo(Context context, Uri input, String output, int startTimeMs,
      int endTimeMs) throws Exception {
    processor(context)
        .input(input)
        .output(output)
  ...
  public static void changeVideoSpeed(Context context, Uri input, String output, float speed)
      throws Exception {
    processor(context)
        .input(input)
        .output(output)
   ...
   */
  public static String reverseVideo(String sourceVideoPath) throws Exception {
    return reverseVideo(sourceVideoPath,true, null);
  }

  /**
   * 对视频先检查，如果不是全关键帧，先处理成所有帧都是关键帧，再逆序
   */
  public static String reverseVideo(String sourceVideoPath,
      boolean reverseAudio, @Nullable VideoProgressListener listener) throws Exception {
    Context context = AppUtil.getApplicationContext();
    String outputPath = FileUtil.getVideoPath(FileUtil.getFileName(sourceVideoPath));
    reverseVideo(context, new MediaSource(context, Uri.parse(sourceVideoPath)), outputPath,
        reverseAudio, listener);
    return outputPath;
  }
   ...
   * 对视频先检查，如果不是全关键帧，先处理成所有帧都是关键帧，再逆序
   */
  public static void reverseVideo(String sourceVideoPath, String outputPath,
      boolean reverseAudio, @Nullable VideoProgressListener listener) throws Exception {
    Context context = AppUtil.getApplicationContext();
    reverseVideo(context, new MediaSource(context, Uri.parse(sourceVideoPath)), outputPath,
        reverseAudio, listener);
   ...
   * 对视频先检查，如果不是全关键帧，先处理成所有帧都是关键帧，再逆序
   */
  public static void reverseVideo(Context context, MediaSource input, String output,
      boolean reverseAudio, @Nullable VideoProgressListener listener) throws Exception {
    ...
    try {
      ...
      if (frameCount == keyFrameCount || frameCount == keyFrameCount + 1) {
        ...
        VideoMultiStepProgress stepProgress =
            new VideoMultiStepProgress(new float[] { 0.45f, 0.1f, 0.45f }, listener);
        ...
        reverseVideoNoDecode(new MediaSource(tempFile.getAbsolutePath()),
            temp2File.getAbsolutePath(), reverseAudio, null, stepProgress);
   ...
   * 支持裁剪缩放快慢放
   */
  public static void processVideo(@NotNull Context context, @NotNull Processor processor)
      throws Exception {
    ...
    MediaMuxer mediaMuxer =
        new MediaMuxer(processor.output, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
    int muxerAudioTrackIndex = 0;
    boolean shouldChangeAudioSpeed =
        processor.changeAudioSpeed == null ? true : processor.changeAudioSpeed;
    ...
    if (audioIndex >= 0) {
      ...
      if (shouldChangeAudioSpeed) {
        ...
        if (processor.startTimeMs != null
          ...
          audioEndTimeMs = (processor.startTimeMs == null ? 0 : processor.startTimeMs) + (int) (
              avDurationUs
                  / 1000);
        }
      }
    ...
    VideoEncodeThread encodeThread = new VideoEncodeThread(extractor, mediaMuxer, processor.bitrate,
        resultWidth, resultHeight, processor.iFrameInterval,
        processor.frameRate == null ? DEFAULT_FRAME_RATE : processor.frameRate, videoIndex,
    ...
    VideoDecodeThread decodeThread =
        new VideoDecodeThread(encodeThread, extractor, processor.startTimeMs, processor.endTimeMs,
            srcFrameRate,
            processor.frameRate == null ? DEFAULT_FRAME_RATE : processor.frameRate, processor.speed,
            processor.dropFrames, videoIndex, decodeDone);

    AudioProcessThread audioProcessThread =
        new AudioProcessThread(context, processor.input, mediaMuxer, processor.startTimeMs,
            audioEndTimeMs,
  ...
  public static void reverseVideoNoDecode(MediaSource input, String output, boolean reverseAudio)
      throws IOException {
   ...
   */
  public static void adjustVideoVolume(Context context, final MediaSource mediaSource,
      final String output, int videoVolume, float faceInSec, float fadeOutSec) throws IOException {
    ...
    int channelCount =
        audioTrackFormat.containsKey(MediaFormat.KEY_CHANNEL_COUNT) ? audioTrackFormat.getInteger(
    ...
    if (faceInSec > 0 || fadeOutSec > 0) {
      AudioFadeUtil.audioFade(videoPcmAdjustedFile.getAbsolutePath(), sampleRate, channelCount,
          faceInSec, fadeOutSec);
    }
    new PcmToWavUtil(sampleRate, channelConfig, channelCount,
        AudioFormat.ENCODING_PCM_16BIT).pcmToWav(videoPcmAdjustedFile.getAbsolutePath(),
        videoWavFile.getAbsolutePath());
    ...
    int rotation = oriVideoFormat.containsKey(MediaFormat.KEY_ROTATION) ? oriVideoFormat.getInteger(
        MediaFormat.KEY_ROTATION) : 0;
    MediaMuxer mediaMuxer = new MediaMuxer(output, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
    ...
    MediaFormat encodeFormat =
        MediaFormat.createAudioFormat(MediaFormat.MIMETYPE_AUDIO_AAC, sampleRate,
            channelCount);//参数对应-> mime type、采样率、声道数
    ...
    try {
      while (!encodeDone) {
        ...
        while (true) {
          ...
          if (outputBufferIndex == MediaCodec.INFO_TRY_AGAIN_LATER) {
            ...
            CL.it(TAG, "audio writeSampleData "
      ...
      int frameRate =
          oriVideoFormat.containsKey(MediaFormat.KEY_FRAME_RATE) ? oriVideoFormat.getInteger(
              MediaFormat.KEY_FRAME_RATE) : (int) Math.ceil(VideoUtil.getAveFrameRate(mediaSource));
      ...
      while (true) {
        ...
        //写入视频
        if (!detectTimeError
            && lastVideoFrameTimeUs != -1
            && info.presentationTimeUs < lastVideoFrameTimeUs + VIDEO_FRAME_TIME_US) {
          //某些视频帧时间会出错
          CL.et(TAG, "video 时间戳错误，lastVideoFrameTimeUs:"
        ...
        CL.wt(TAG, "video writeSampleData:"
            + info.presentationTimeUs
            + " type:"
            + info.flags
            + " size:"
  ...
  public static String getPath(Context context, String sourceVideoPath) {
    context.getCacheDir();
    File file = new File(context.getCacheDir() + "/mix/" + FileUtil.getFileName(sourceVideoPath));
   ...
   * 只支持16bit音频
   */
  public static String mixAudioTrack(String sourceVideoPath, String sourceAudioPath)
      throws IOException {
    Context context = AppUtil.getApplicationContext();
    int duration = FileUtil.getDuration(sourceVideoPath);
    String path = getPath(context, sourceVideoPath);
    mixAudioTrack(context, new MediaSource(context, Uri.parse(sourceVideoPath)),
        new MediaSource(sourceAudioPath), path, 0, duration, 100, 100, 1f, 1f);
    return path;
  }
   ...
   * 只支持16bit音频
   */
  public static void mixAudioTrack(String sourceVideoPath, String sourceAudioPath,
      final String outputPath) throws IOException {
    Context context = AppUtil.getApplicationContext();
    int duration = FileUtil.getDuration(sourceVideoPath);
    mixAudioTrack(context, new MediaSource(context, Uri.parse(sourceVideoPath)),
        new MediaSource(sourceAudioPath), outputPath, 0, duration, 100, 100, 1f, 1f);
  }
   ...
   */
  public static String mixAudioTrack(String sourceVideoPath, String sourceAudioPath,
      Integer startTimeMs, Integer endTimeMs) throws IOException {
    Context context = AppUtil.getApplicationContext();
    context.getCacheDir();
    File file = new File(context.getCacheDir() + "/mix/" + FileUtil.getFileName(sourceVideoPath));
    String outPath = file.getAbsolutePath();
    FileUtil.createOrExistsFile(file);
    mixAudioTrack(context, new MediaSource(context, Uri.parse(sourceVideoPath)),
        new MediaSource(sourceAudioPath), outPath, startTimeMs, endTimeMs, 100, 100, 1f, 1f);
    return outPath;
  }
  ...
  public static void mixAudioTrack(String sourceVideoPath, String sourceAudioPath,
    ...
    Context context = AppUtil.getApplicationContext();
    mixAudioTrack(context, new MediaSource(context, Uri.parse(sourceVideoPath)),
        new MediaSource(sourceAudioPath), outputPath, startTimeMs, endTimeMs, 100, 100, 1f, 1f);
  }
  ...
  public static void mixAudioTrack(String sourceVideoPath, String sourceAudioPath,
    ...
    Context context = AppUtil.getApplicationContext();
    mixAudioTrack(context, new MediaSource(context, Uri.parse(sourceVideoPath)),
        new MediaSource(sourceAudioPath), outputPath, startTimeMs, endTimeMs, videoVolume,
        aacVolume, fadeInSec, fadeOutSec);
  }
   ...
   * @param aacVolume 0静音，100表示原音
   */
  public static void mixAudioTrack(Context context, final MediaSource videoInput,
      final MediaSource audioInput, final String output,
      Integer startTimeMs, Integer endTimeMs,
      int videoVolume,
      int aacVolume,
      float fadeInSec, float fadeOutSec) throws IOException {
    ...
    if (oriAudioIndex >= 0) {
      ...
      new Thread(new Runnable() {
        ...
        public void run() {
          try {
            AudioUtil.decodeToPCM(videoInput, videoPcmFile.getAbsolutePath(), startTimeUs,
                endTimeUs);
          } catch (IOException e) {
      ...
      new Thread(new Runnable() {
        ...
        public void run() {
          try {
            AudioUtil.decodeToPCM(audioInput, finalAacPcmFile.getAbsolutePath(), 0,
      ...
      Pair<Integer, Integer> resultPair =
          AudioUtil.checkAndAdjustAudioFormat(videoPcmFile.getAbsolutePath(),
      ...
      if (fadeInSec != 0 || fadeOutSec != 0) {
        AudioFadeUtil.audioFade(adjustedPcm.getAbsolutePath(), sampleRate, channelCount, fadeInSec,
            fadeOutSec);
      }
      //PCM转WAV
      new PcmToWavUtil(sampleRate, channelConfig, channelCount,
      ...
      CL.et("hwLog",
          String.format("decode:%dms,resample:%dms,mix:%dms,fade:%dms", s2 - s1, s3 - s2, s4 - s3,
              s5 - s4));
      ...
      if (channelCount > 2) {
        ...
        AudioUtil.stereoToMonoSimple(aacPcmFile.getAbsolutePath(), tempFile.getAbsolutePath(),
            channelCount);
        channelCount = 1;
        aacPcmFile.delete();
        aacPcmFile = tempFile;
      ...
      if (aacVolume != 50) {
        adjustedPcm = new File(cacheDir, "adjusted_" + System.currentTimeMillis() + ".pcm");
        AudioUtil.adjustPcmVolume(aacPcmFile.getAbsolutePath(), adjustedPcm.getAbsolutePath(),
            aacVolume);
      } else {
    ...
    try {
      ...
      int frameRate =
          oriVideoFormat.containsKey(MediaFormat.KEY_FRAME_RATE) ? oriVideoFormat.getInteger(
              MediaFormat.KEY_FRAME_RATE) : (int) Math.ceil(VideoUtil.getAveFrameRate(videoInput));
      buffer = ByteBuffer.allocateDirect(maxBufferSize);
      final int VIDEO_FRAME_TIME_US = (int) (1000 * 1000f / frameRate);
      long lastVideoFrameTimeUs = -1;
      detectTimeError = false;
  ...
  public static Processor processor() {
    return new Processor(AppUtil.getApplicationContext());
  }
  ...
  public static class Processor {
    ...
    public String process() throws Exception {
      if (TextUtils.isEmpty(this.outPath)) {
        this.outPath = getPath(context, this.input.inputPath);
      }
