...
/**
 * created by jiangshide on 2019-12-12.
 * email:18311271399@163.com
 */
public class ZdSideBar extends View {

  private final static int DEFAULT_TEXT_SIZE = 14; // sp
  private final static int DEFAULT_MAX_OFFSET = 80; //dp

  private final static String[] DEFAULT_INDEX_ITEMS = {
      "A", "B", "C", "D", "E", "F", "G", "H", "I",
  ...
  public ZdSideBar(Context context) {
    this(context, null);
  }

  public ZdSideBar(Context context, AttributeSet attrs) {
    this(context, attrs, 0);
  }

  public ZdSideBar(Context context, AttributeSet attrs, int defStyleAttr) {
    super(context, attrs, defStyleAttr);
    mDisplayMetrics = context.getResources().getDisplayMetrics();

    TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.ZdSideBar);
    mLazyRespond = typedArray.getBoolean(R.styleable.ZdSideBar_sidebar_lazy_respond, false);
    mTextColor = typedArray.getColor(R.styleable.ZdSideBar_sidebar_text_color, Color.GRAY);
    mTextSize =
        typedArray.getDimension(R.styleable.ZdSideBar_sidebar_text_size, sp2px(DEFAULT_TEXT_SIZE));
    mMaxOffset = typedArray.getDimension(R.styleable.ZdSideBar_sidebar_max_offset,
        dp2px(DEFAULT_MAX_OFFSET));
    mSideBarPosition = typedArray.getInt(R.styleable.ZdSideBar_sidebar_position, POSITION_RIGHT);
    mTextAlignment =
        typedArray.getInt(R.styleable.ZdSideBar_sidebar_text_alignment, TEXT_ALIGN_CENTER);
    typedArray.recycle();
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    float areaLeft =
        (mSideBarPosition == POSITION_LEFT) ? 0 : (width - mBarWidth - getPaddingRight());
    float areaRight =
        (mSideBarPosition == POSITION_LEFT) ? (getPaddingLeft() + areaLeft + mBarWidth) : width;
  ...
  private float getItemScale(int index) {
    float scale = 0;
    if (mCurrentIndex != -1) {
      float distance = Math.abs(mCurrentY - (mIndexItemHeight * index + mIndexItemHeight / 2))
          / mIndexItemHeight;
